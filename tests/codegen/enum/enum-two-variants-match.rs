...
// For two-variant niched enums, we want to avoid testing for the *niched* value,
// since in a sense it doesn't matter, but if we mention it then sometimes LLVM
// will think that its specific value is important, and not optimize away the
// Option the way we want, see https://github.com/llvm/llvm-project/issues/132678

// Instead, check for the *payload*'s range, since that's still cheap and makes
// it more obvious that in the `Some` case for `Option<bool>`, for example,
// any further processing only needs to handle `false`/`true`, not also `3`.
