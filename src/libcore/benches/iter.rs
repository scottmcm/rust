...
struct NaiveIter<I: Iterator>(I);
impl<I:Iterator> Iterator for NaiveIter<I> {
    type Item = I::Item;
    fn next(&mut self) -> Option<Self::Item> { self.0.next() }
}

/// Helper to benchmark `sum` for iterators with and without
/// the optimizations on `fold`
macro_rules! bench_sums {
    ($bench_sum:ident, $bench_naive_sum:ident, $iter:expr) => {
        #[bench]
        ...
        #[bench]
        fn $bench_naive_sum(b: &mut Bencher) {
            b.iter(|| -> i64 {
                NaiveIter($iter.map(black_box)).sum()
            });
        }
    }
}
